...
M <- 2 # fit two dimensional
stan_rdump(c("x", "w", "z", "sigma", "N", "D", "M"), file="out/oned_ppca.data.R")
oned_ppca <- read_rdump("out/oned_ppca.data.R")

z = as.matrix(expand.grid(linspace(-2, 2, n = 50), linspace(-2, 2, n = 50)))

oned_ppca_ll_zs_true = apply(z, 1, ppca_ll, params_w=cbind(w, rep(0, D)), params_logsigma=0, input_data=oned_ppca)

write.csv(oned_ppca_ll_zs_true, "out/oned_ppca_ll_zs_true.csv")


stan_file = 'ppca_mix.stan'
# fit by hmc
degenerate_fit_hmc <- stan(file=stan_file, data=oned_ppca,
                       chains=4, seed=483892929, refresh=2000)

# # fit by vb
# gauss_mix_stan = stan_model(file=stan_file, verbose=TRUE)
# degenerate_fit_vbmf <- vb(gauss_mix_stan, data=oned_ppca, 
# 	tol_rel_obj = 1e-3, algorithm ="meanfield")

# degenerate_fit_vbfr <- vb(gauss_mix_stan, data=oned_ppca, 
...
for (i in 1:length(fits)){
	...
	write.csv(theta_sample, paste(c("out/oned_ppca_z_sample_", method, ".csv"), collapse = ''))

...
stan_rdump(c("x", "w", "z", "sigma", "N", "D", "M"), file="out/twod_ppca.data.R")
twod_ppca <- read_rdump("out/twod_ppca.data.R")

z = as.matrix(expand.grid(linspace(-2, 2, n = 50), linspace(-2, 2, n = 50)))

twod_ppca_ll_zs_true = apply(z, 1, ppca_ll, params_w=w, params_logsigma=0, input_data=twod_ppca)

write.csv(twod_ppca_ll_zs_true, "out/twod_ppca_ll_zs_true.csv")
...
for (i in 1:length(fits)){
	...
	write.csv(theta_sample, paste(c("out/twod_ppca_z_sample_", method, ".csv"), collapse = ''))

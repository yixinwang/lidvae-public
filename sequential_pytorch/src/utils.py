...
def reconstruct_li_et_al(model, test_data_batch, vocab, strategy, fname):
    hyps = []
    refs = []
    with open(fname, "w") as fout:
        #for i in range(10):
...
def test_li_et_al_ae(model, test_data_batch, mode, args, verbose=True):
    global logging
...
def test_li_et_al_vae(model, test_data_batch, mode, args, verbose=True):
    global logging
...
def calc_mi_li_et_al(model, test_data_batch):
    # calc_mi_v3
    import math 
...
def visualize_latent_li_et_al(args, epoch, vae, device, test_data):
    nsamples = 1
